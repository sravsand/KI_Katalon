...
String[] newTimesheet = ["Non-Project Work", "Meetings", ""]
String[] newHours = ["8.00", "", "8.00", "Undo", "8.00", "", "8.00", "", "8.00", "", "", "", "", ""]
...
Rep.nextTestStep("Select project, activity and task on new row")
	Pop.CreateAdditionalRow("3", timesheet.row[0].project, timesheet.row[0].activity, timesheet.row[0].task)

	
//Test Step
Rep.nextTestStep("Populate Timesheet")
	Pop.StandardHoursAdditionalRowModel(timesheet, "1", 0)
	
	
//Test Step
Rep.nextTestStep("Click on [Cancel] icon")
	gObj.buttonClickSync(Const.timesheet + 'TimesheetCancel')
	gAct.Wait(2)
