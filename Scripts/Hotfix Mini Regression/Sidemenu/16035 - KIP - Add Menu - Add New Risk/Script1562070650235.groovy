...
String [] newRisk = ["Test Risk from sidemenu" + browser, 'Arch Now Web Services Implementation', GVars.user, "Open", todayDate, impDate, "Project", "High", "Medium", "High", resDate, "", ""]
...
Rep.nextTestStep("Hover over the mouse to the [+] sign from the side bar menu and select risk")
	WebUI.mouseOver(findTestObject(Const.plusMenu + 'plusSideMenu'))	
	gObj.buttonClickSync(Const.plusMenu + 'menu_Risk')
...
Rep.nextTestStep("Click [Filter] to display Default Risk Filter")
	gObj.buttonClickSync(Const.risks + 'filterDropdown')
	gObj.buttonClickSync(Const.risks + 'a_DefaultRiskFilter')
