...
Rep.nextTestStep("Click on [Save] or [Save & Close] button")
	...
	gAct.compareStringAndReport(errorMsg, "Unable to Save Department")
	
	
Rep.nextTestStep("Add text in [Name] field")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Name"), department.name)
	
	
Rep.nextTestStep("Add [Code]")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Code"), department.code)
