...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String clonedSecurityGroupName = Com.generateRandomText(8)
	String clonedSecurityGroupCode = clonedSecurityGroupName.toUpperCase()
	
	String[] newClonedSecurityGroup = [clonedSecurityGroupName, clonedSecurityGroupCode]
	SecurityGroup clonedSecurityGroup = GenerateSecurityGroup.createSecurityGroup(newClonedSecurityGroup)
...
Rep.nextTestStep("Amend text in [Name] field")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Name"), clonedSecurityGroup.name)
	
	
Rep.nextTestStep("Add text in [Code] field")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Code"), clonedSecurityGroup.code)
...
Rep.nextTestStep("Verify the cloned [Activity] is available for selection in the table")
	sVal.searchTableAddedValue(3, clonedSecurityGroup.name)
