...
Rep.nextTestStep("Select [Risks] from search filter")
	...
	Boolean valueBefore = WebUI.verifyElementNotPresent(findTestObject(Const.columnPicker + "header_Probability"), 5)
	gAct.Wait(2)
...
Rep.nextTestStep("Click on [...] from menu bar ")
	...
	gAct.Wait(2)
	WebUI.dragAndDropToObject(findTestObject(Const.columnPicker + "col_Probability"), findTestObject(Const.columnPicker + "col_Owner"))

	gObj.buttonClickSync(Const.columnPicker + "button_Save")
	gAct.Wait(GVars.shortWait)
	
	Boolean value2After = WebUI.verifyElementPresent(findTestObject(Const.columnPicker + "header_Probability"), 5)
