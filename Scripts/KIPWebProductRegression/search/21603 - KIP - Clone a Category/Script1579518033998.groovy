...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String cloneCategoryName = Com.generateRandomText(10)
	String cloneCategoryCode = cloneCategoryName.toUpperCase()
	
	String[] newCloneCategory = [cloneCategoryName, cloneCategoryCode, "true"]
	Category cloneCategory = GenerateCategory.createCategory(newCloneCategory)
...
Rep.nextTestStep("Select [Categories] from dropdown list")
	WebUI.click(findTestObject(Const.search + "filterType", [('label'): searchItem]))
	gAct.Wait(1)


Rep.nextTestStep("Click onto [Inline menu] of an Categories")
...
Rep.nextTestStep("Amend text in [Name] field")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Name"), cloneCategory.name)

	
Rep.nextTestStep("Add text in [Code] field")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Code"), cloneCategory.code)
...
Rep.nextTestStep("Verify the cloned [Activity] is available for selection in the table")
	sVal.searchTableAddedValue(3, cloneCategory.name)
