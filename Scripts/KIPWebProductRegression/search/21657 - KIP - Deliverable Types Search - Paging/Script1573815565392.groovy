...
Rep.nextTestStep("Select [Deliverable Types] from search filter")
	...
	String textVal = WebUI.getAttribute(findTestObject(Const.columnPicker + "pageCount"), 'value')
	if(textVal != "10"){
		gObj.selectComboByLabelSync(Const.columnPicker + "pageCount", "10")
	}
	

Rep.nextTestStep("In table footer select [5] from dropdown")
	gObj.selectComboByLabelSync(Const.columnPicker + "pageCount", "5")
	
	
Rep.nextTestStep("Click [>>] from table footer  &  Click [<<] from table footer")
