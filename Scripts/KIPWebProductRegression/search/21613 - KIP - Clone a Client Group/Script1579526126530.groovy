...
String searchItem = "Customer Groups"
String searchVal = "Customer Group"
...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String cloneClientGroupName = Com.generateRandomText(10)
	String cloneClientGroupCode = cloneClientGroupName.toUpperCase()
	
	String[] newClonedClientGroup = [cloneClientGroupName, cloneClientGroupCode]
	ClientGroup cloneClientGroup = GenerateClientGroup.createClientGroup(newClonedClientGroup)
...
Rep.nextTestStep("Select [Customer Groups] from dropdown list")
	WebUI.click(findTestObject(Const.search + "filterType", [('label'): searchItem]))
	gAct.Wait(1)


Rep.nextTestStep("Click onto [Inline menu] of an Customer Groups")
...
Rep.nextTestStep("Amend text in [Name] field")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Name"), cloneClientGroup.name)

	
Rep.nextTestStep("Add text in [Code] field")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Code"), cloneClientGroup.code)
...
Rep.nextTestStep("Verify the cloned [Activity] is available for selection in the table")
	sVal.searchTableAddedValue(3, cloneClientGroup.name)
