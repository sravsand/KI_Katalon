...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String notes = Com.generateRandomText(10)
	...
	String[] newHours = ["7.00", notes, "7.00", notes, "7.00", notes, "7.00", notes, "7.00", notes, "", "", "", ""]
	Timesheet timesheet = GenerateTimesheet.createTimesheet(thisWeek, newTimesheet, newHours)
	component.submitOldTimesheetAndApprove_SpecificUser(timesheet, 0, weekCommence, resource)
...
Rep.nextTestStep("Verify the [Status] of Timesheet")
	String actApproved = gObj.getEditTextSync(Const.columnPicker + "subMissionStatus")
	gAct.findSubstringInStringAndReport(actApproved, "Accepted (DLEESWF " + todaysDate)


Rep.nextTestStep("Click on [Unlock] next to Status")
	gObj.buttonClickSync(Const.columnPicker + "btnUnlock")
...
Rep.nextTestStep("Click [Unlock]")
	gObj.buttonClickSync(Const.columnPicker + "button_Unlock")
	gAct.Wait(GVars.shortWait)
	
	
Rep.nextTestStep("Verify the [Status] of the timesheet changed to [Unlocked] ")
	String actNewApproved = gObj.getEditTextSync(Const.columnPicker + "subMissionStatus")
	gAct.findSubstringInStringAndReport(actNewApproved, "Unlocked (DLEESWF " + todaysDate)
...
Rep.nextTestStep("Verify the [Status] of the timesheet changed to [Rejected] on Timesheet Submissions table")
	...
	gAct.compareStringAndReport(newStatus, "Unlocked")
