...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String[] newExchangeRate = [currency.code, todaysDate, "3.50"]
...
Rep.nextTestStep("Click onto [Adjust Expense Exchange Rates] option")
	WebUI.click(findTestObject(Const.financialsMenu + "a_AdjustExpenseExchangeRates"))
	gAct.Wait(2)
	WebUI.switchToDefaultContent()

	
Rep.nextTestStep("Select a [Date Between] via date picker")
	WebUI.setText(findTestObject(Const.adjustExpenseExRate + "input_FromDate"), todaysDate)

	
Rep.nextTestStep("Select a [Date To] via date picker")
	WebUI.setText(findTestObject(Const.adjustExpenseExRate + "input_ToDate"), weekEnd)

	
Rep.nextTestStep("Select [Expense Cost Amount]")
	WebUI.check(findTestObject(Const.adjustExpenseExRate + "chkCostAmount"))

	
Rep.nextTestStep("Select [Expense Charge Amount]")
	WebUI.check(findTestObject(Const.adjustExpenseExRate + "chkChargeAmount"))

	
Rep.nextTestStep("Click onto [Adjust] button")
	WebUI.click(findTestObject(Const.adjustExpenseExRate + "btnAdjust"))
