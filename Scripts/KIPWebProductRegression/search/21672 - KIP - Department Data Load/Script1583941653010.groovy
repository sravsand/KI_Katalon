...
String deptName = Com.generateRandomText(5)
String deptCode = deptName.toUpperCase()

String downloadPath = "C:\\Katalon Studio\\KeyedInProjects\\downloads\\KIP_Department_DataLoad.xls"
...
Rep.nextTestStep("Verify the spreadsheet have all the columns selected at Step 6")
	...
	gAct.compareStringAndReport(colOne, "Department Code")
	String colTwo = ExportData.getValue(3, 10)
	gAct.compareStringAndReport(colTwo, "Department Name")
...
Rep.nextTestStep("Fill the [Mandatory] fields in spreadsheet")
	eAct.writeToCellInExcelXLS(downloadPath, searchItem, 10, 1, deptName)
	eAct.writeToCellInExcelXLS(downloadPath, searchItem, 10, 2, deptCode)
...
Rep.nextTestStep("Click [Search] from [Template Customer] and Select an [Customer]")
	gObj.setEditSync(Const.dataLoad + "input_TemplateActivity", "DEPT_HLEZZM")
...
Rep.nextTestStep("Verify the recently upload data is available for selection in Activity table")
	sVal.searchTableAddedValue(2, deptCode)
