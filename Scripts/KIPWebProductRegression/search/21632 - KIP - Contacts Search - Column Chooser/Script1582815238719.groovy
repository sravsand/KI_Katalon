...
Rep.nextTestStep("Select [Contacts] from search filter")
	...
	gAct.Wait(1)
	Boolean valueBefore = WebUI.verifyElementNotPresent(findTestObject(Const.columnPicker + "header_Key"), 5)
	gAct.Wait(2)
...
Rep.nextTestStep("Click on [...] from menu bar ")
	...
	gAct.Wait(2)
	WebUI.dragAndDropToObject(findTestObject(Const.columnPicker + "col_Key"), findTestObject(Const.columnPicker + "col_Email"))
	gObj.buttonClickSync(Const.columnPicker + "button_Save")
	
	Boolean value2After = WebUI.verifyElementPresent(findTestObject(Const.columnPicker + "header_Key"), 5)
