...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String[] newForecast = [forecastName, "Project Innovation_CHROME_DRIVER", "KIPDEV", "KIP1TEST", benefitName, "Reduction in Costs", newDate]
//	String[] newForecast = [forecastName, "Project Innovation_" + selectedBrowser, "KIPDEV", "KIP1TEST", benefitName, "Reduction in Costs", todaysDate]
...
Rep.nextTestStep("Click onto [Inline menu] of Forecasts")
	...
	gObj.buttonClickSync(Const.columnPicker + "btn_ApproveForecast")
	gAct.Wait(1)
	WebUI.refresh()
	gAct.Wait(GVars.shortWait)
	
	
Rep.nextTestStep("Click onto [baseline]")
	rowNo = sVal.searchTableReturnRow(2, forecast.name)
	gObj.buttonClickSubIntPosSync(Const.columnPicker + 'inlineEdit_Alt', rowNo, "row")
	gObj.buttonClickSubIntPosSync(Const.columnPicker + "a_InlineMenuPositionSelection", 2, "pos")
...
Rep.nextTestStep("Verify the baseline column of the Forecast change  to [Yes]")
	WebUI.refresh()
	gAct.Wait(GVars.shortWait)
	rowNo = sVal.searchTableReturnRow(2, forecast.name)
	String newStatus = gObj.getEditTextDblSubIntPosSync(Const.columnPicker + "searchTable", rowNo, "row", 4, "col")
	gAct.compareStringAndReport(newStatus, "Yes")
