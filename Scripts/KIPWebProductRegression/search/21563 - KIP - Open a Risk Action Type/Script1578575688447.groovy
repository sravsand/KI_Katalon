...
Rep.nextTestStep("Change the date in [Expected Resolution Date]")
	gObj.setEditSync(Const.risks + 'input_ResolutionDate', newResDate)
...
Rep.nextTestStep("Verify that the Risks table has been updated with new expected resolution date")
	int rowNum = sVal.searchTableReturnRow(3, risk.title)
	String expDate = gObj.getEditTextDblSubIntPosSync(Const.columnPicker + "searchTable", rowNum, "row", 8, "col")
	gAct.compareStringAndReport(expDate, newResDate)
