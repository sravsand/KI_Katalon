...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String clonedSkillName = Com.generateRandomText(10)
	String clonedSkillCode = clonedSkillName.toUpperCase()
	
	String[] newClonedSkill = [clonedSkillName, clonedSkillCode, "1", "6"]
	Skill clonedSkill = GenerateSkill.createSkill(newClonedSkill)
...
Rep.nextTestStep("Amend text in [Name] field")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Name"), clonedSkill.name)
	
	
Rep.nextTestStep("Add text in [Code] field")
	WebUI.setText(findTestObject(Const.columnPicker + "input_Code"), clonedSkill.code)
...
Rep.nextTestStep("Verify the cloned [Activity] is available for selection in the table")
	sVal.searchTableAddedValue(3, clonedSkill.name)
