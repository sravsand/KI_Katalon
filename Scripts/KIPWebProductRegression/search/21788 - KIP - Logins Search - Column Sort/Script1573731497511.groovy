...
Rep.nextTestStep("Select [Logins] from search filter")
	...
	gObj.buttonClickSync(Const.columnPicker + "header_OwnResource")
...
Rep.nextTestStep("Click on [Own Resource] from menu bar")
	gObj.buttonClickSync(Const.columnPicker + "header_OwnResource")
	def preSortCode = sAct.searchTableGetColumnValues_Alt(6)
	gObj.buttonClickSync(Const.columnPicker + "header_OwnResource")
	def postSortCode = sAct.searchTableGetColumnValues_Alt(6)
	boolean matchCode = gVal.matchingReverseArrays(preSortCode, postSortCode)
	
	if(matchCode){
		Rep.pass("Own Resource  has been sorted correctly.")
	}else{
		Rep.fail("Own Resource has not been sorted correcly.")
	}
