...
Rep.nextTestStep("Select [Invoices] from search filter")
	...
	gAct.Wait(1)
	Boolean valueBefore = WebUI.verifyElementNotPresent(findTestObject(Const.columnPicker + "header_Customer"), GVars.midWait)
...
Rep.nextTestStep("Click on [...] from menu bar ")
	...
	gAct.Wait(2)
	WebUI.dragAndDropToObject(findTestObject(Const.columnPicker + "col_Customer"), findTestObject(Const.columnPicker + "col_Key"))
	gObj.buttonClickSync(Const.columnPicker + "button_Save")
	
	Boolean value2After = WebUI.verifyElementPresent(findTestObject(Const.columnPicker + "header_Customer"), 5)
