...
Rep.nextTestStep("Select [Working Times] from search filter")
	...
	Boolean valueBefore = WebUI.verifyElementNotPresent(findTestObject(Const.columnPicker + "header_Code"), GVars.shortWait)
...
Rep.nextTestStep("Click on [...] from menu bar ")
	...
	gAct.Wait(2)
	WebUI.dragAndDropToObject(findTestObject(Const.columnPicker + "col_Code"), findTestObject(Const.columnPicker + "col_Name"))
	gObj.buttonClickSync(Const.columnPicker + "button_Save")
	
	Boolean value2After = WebUI.verifyElementPresent(findTestObject(Const.columnPicker + "header_Code"), GVars.shortWait)
