...
Rep.nextTestStep("Select [Contract] from search filter")
	...
	gAct.Wait(1)
	Boolean valueBefore = WebUI.verifyElementNotPresent(findTestObject(Const.columnPicker + "header_Project"), GVars.midWait)
...
Rep.nextTestStep("Click on [...] from menu bar ")
	...
	gAct.Wait(2)
	WebUI.dragAndDropToObject(findTestObject(Const.columnPicker + "col_Project"), findTestObject(Const.columnPicker + "col_Customer"))
	gObj.buttonClickSync(Const.columnPicker + "button_Save")
	
	Boolean value2After = WebUI.verifyElementPresent(findTestObject(Const.columnPicker + "header_Project"), GVars.midWait)
