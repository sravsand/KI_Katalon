...
Rep.nextTestStep("Select [Contacts] from search filter")
	...
	gObj.buttonClickSync(Const.columnPicker + "header_Contact")
	gAct.Wait(2)
	
	
Rep.nextTestStep("Click on [Customer] from menu bar")
	WebD.clickElement("//th[@class='sorting'][contains(text(),'Customer')]")
	...
	if(match){
		Rep.pass("Customer has been sorted correctly.")
	}else{
		Rep.fail("Customer has not been sorted correcly.")
	}
	
	
Rep.nextTestStep("Click on [Contact] from menu bar")
	gObj.buttonClickSync(Const.columnPicker + "header_Contact")
	...
	if(matchCode){
		Rep.pass("Contact has been sorted correctly.")
	}else{
		Rep.fail("Contact has not been sorted correcly.")
	}
