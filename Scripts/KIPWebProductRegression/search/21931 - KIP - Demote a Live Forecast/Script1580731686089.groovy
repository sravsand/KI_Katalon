...
Rep.nextTestStep("Click onto [Unapprove]")
	rowNo = sVal.searchTableReturnRow(2, forecast.name)
	gObj.buttonClickSubIntPosSync(Const.columnPicker + 'inlineEdit_Alt', rowNo, "row")
	gObj.buttonClickSubIntPosSync(Const.columnPicker + "a_InlineMenuPositionSelection", 3, "pos")
...
Rep.nextTestStep("Verify the status of the Forecast change as [Former Live]")
	gObj.refreshUI()
	rowNo = sVal.searchTableReturnRow(2, forecast.name)
	String newStatus = gObj.getEditTextDblSubIntPosSync(Const.columnPicker + "searchTable", rowNo, "row", 3, "col")
	gAct.compareStringAndReport(newStatus, "Former Live")
