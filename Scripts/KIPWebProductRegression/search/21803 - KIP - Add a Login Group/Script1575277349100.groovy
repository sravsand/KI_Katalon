...
Rep.nextTestStep("Click on [Save] or [Save & Close] button")
	...
	gAct.compareStringAndReport(errorMsg, "Unable to Save Login Group")
...
Rep.nextTestStep("Click on [Action Views] tab within [Add Login Groups] modal")
...
Rep.nextTestStep("Select the [Action Views] user want to add through checkbox")
	WebUI.check(findTestObject(Const.columnPicker + "a_All Actions"))
	
	
Rep.nextTestStep("Click on [General] tab within [Add Login Groups] modal")
	WebUI.click(findTestObject(Const.columnPicker + "td_General"))
	
	
Rep.nextTestStep("Click on [Default Action Views] dropdown & Select a [Action View] from dropdown list")
...
Rep.nextTestStep("Verify the newly added Expense Type is added onto the table")
	sVal.searchTableAddedValue(2, loginGroup.code)
