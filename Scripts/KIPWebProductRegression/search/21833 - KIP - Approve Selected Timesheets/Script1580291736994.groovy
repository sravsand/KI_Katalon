...
Rep.nextTestStep("Select the [Timesheets] through checkbox user want to approve")
	String actualValue
...
Rep.nextTestStep("Click [Approve]")
	gObj.buttonClickSync(Const.columnPicker + "a_Approve")


Rep.nextTestStep("Click [Save]")
	gObj.buttonClickSync(Const.columnPicker + "input_SaveApproveTime")
	gAct.Wait(GVars.shortWait)
	

Rep.nextTestStep("Verify the selected timesheet is marked as [Yes] in [Approve] column")

	for(int intCnt = 1; intCnt < 6; intCnt ++){
		String usr = gObj.getEditTextDblSubIntPosSync(Const.columnPicker + "searchTable", intCnt, "row", 4, "col")
		if(usr == resource.name){
			String Active_new = gObj.getEditTextDblSubIntPosSync(Const.columnPicker + "searchTable", intCnt, "row", 13, "col")
