...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String endDate = Com.increaseWorkingCalendarWithFormat(today, 14, pattern)
...
Rep.nextTestStep("Click onto [Drop]")
	gObj.selectInlineOption(8)
	

Rep.nextTestStep("Verify the [Status] is [Drop]")
	String status = gObj.getAttributeSync(Const.columnPicker + "select_DropReschedule", 'value')
	gAct.compareStringAndReport(status, "3")
...
Rep.nextTestStep("Verify [Blue Cross] is appearing against the above deliverable")
	gObj.buttonClickSync(Const.columnPicker + "a_New Filter")
	gObj.buttonClickSync(Const.columnPicker + "a_Search")
	int rowNum = sVal.searchTableReturnRow(5, deliverable.name)
	WebUI.verifyElementPresent(findTestObject(Const.columnPicker + 'sym_Dropped', [('row'): rowNum]), GVars.midWait)
