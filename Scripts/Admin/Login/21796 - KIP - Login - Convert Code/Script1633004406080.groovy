...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String newLoginCode = Com.generateRandomText(7).toUpperCase()
...
Rep.nextTestStep("Enter code in [New Code] textbox")
	gObj.setEditSync(Const.kip4CodeConverter + "input_newCode", newLoginCode)
	
	
Rep.nextTestStep("Click on [Convert]")
	gObj.buttonClickSync(Const.kip4CodeConverter + "btn_Convert")
	gObj.buttonClickSync(Const.kip4CodeConverter + "btn_OK")
...
Rep.nextTestStep("Verify that the changes have been updated in expense type table")
	gObj.createNewFilter("Login Name", "=", login.name)
	sVal.searchTableAddedValue(2, newLoginCode)
