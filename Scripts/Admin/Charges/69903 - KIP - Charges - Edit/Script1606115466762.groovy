...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String[] edtCharge = [edtChargeName, chargeCode, "111111.99", currency.name]
...
Rep.nextTestStep("Attempt entry of text in [Code] field (Required)")
...
Rep.nextTestStep("Select [Currency] from dropdown (Required)")
	gObj.selectComboboxValue("currency", "currency", newEdtCharge.currency, newEdtCharge.currency)

		
Rep.nextTestStep("Enter numerical value in [Rate] field")
...
Rep.nextTestStep("Verify the changes are updated accurately on the charge table")
	sVal.searchTableAddedValue(2, newEdtCharge.code)
	
	int rowNum = sVal.searchTableReturnRow(2, newEdtCharge.code)
	
	String actRateVal = gObj.getEditTextDblSubIntPosSync(Const.columnPicker + "searchTable", rowNum, "row", 4, "col")
	gAct.compareStringAndReport(actRateVal, newEdtCharge.rate)
