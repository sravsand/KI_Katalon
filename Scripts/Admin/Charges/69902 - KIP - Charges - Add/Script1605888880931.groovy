...
Rep.nextTestStep("Add a [Name]")
	gObj.setEditSync(Const.kip4Generic + "input_Name", charge.name)

	
Rep.nextTestStep("Add a [Code] ")
	gObj.setEditSync(Const.kip4Generic + "input_Code", charge.code)
...
Rep.nextTestStep("Verify the [Currency] is pre-populated with system default currency")
	gObj.selectComboboxValue("currency", "currency", charge.currency, charge.currency)


Rep.nextTestStep("Add [Rate]")
	gObj.setEditSync(Const.kip4Cost + "input_rate", charge.rate)
...
Rep.nextTestStep("Verify the changes are updated accurately on the charge table")
	sVal.searchTableAddedValue(2, charge.code)
