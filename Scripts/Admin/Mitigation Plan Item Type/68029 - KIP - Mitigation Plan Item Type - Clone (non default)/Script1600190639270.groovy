...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String clonedMitigationPlanItemTypeName = Com.generateRandomText(8)
	String clonedMitigationPlanItemTypeCode = clonedMitigationPlanItemTypeName.toUpperCase()
	
	String[] newClonedMitPlanType = [clonedMitigationPlanItemTypeName, clonedMitigationPlanItemTypeCode]
	MitigationPlanItemType newClonedMitigationPlanItemType = GenerateMitigationPlanItemType.createMitigationPlanItemType(newClonedMitPlanType)
...
Rep.nextTestStep("Click onto [Cloned]")
	gObj.selectInlineOption(2)

		
Rep.nextTestStep("Enter text in 'Code' field")
	gObj.setEditSync(Const.kip4Generic + "input_Code", newClonedMitigationPlanItemType.code)


Rep.nextTestStep("Leave Default Mitigation Plan Item Type checkbox unchecked")
	def mitVal = gObj.getAttributeSync(Const.kip4MitigationPlanItemType + "chk_DefaultMitigationPlanItemType", 'value')
	gAct.compareStringAndReport(mitVal, null)
...
Rep.nextTestStep("Verify the changes are updated accurately on the location table")
	sVal.searchTableAddedValue(2, newClonedMitigationPlanItemType.code)
