...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String[] clonedNewCost = [clonedCostName, clonedCostCode, "11.00", "", currency.name]
	Cost clonedCost = GenerateCost.createCost(clonedNewCost)
...
Rep.nextTestStep("Add a [Code] ")
	gObj.setEditSync(Const.kip4Generic + "input_Code", clonedCost.code)
	
	
Rep.nextTestStep("Amend a [name] ")
	gObj.setEditSync(Const.kip4Generic + "input_Name", clonedCost.name)
...
Rep.nextTestStep("Verify the changes are updated accurately on the cost table")
	sVal.searchTableAddedValue(2, clonedCost.code)
