...
Rep.nextTestStep("Click onto [Delete]")
	...
	if(modalError){
		...
		gAct.findSubstringInStringAndReport(errtext, "It is not possible to delete '" + nominalCode.name + "'")
	}
	gObj.buttonClickSync(Const.kip4DeleteModal + "button_Cancel")


Rep.nextTestStep("Verify the deleted currency is not available for selction in currency table")
