...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String[] newExpenseType = [expenseName, expenseCode, "", "1", "true", "10.00"]
	ExpenseType expenseType = GenerateExpenseType.createExpenseType(newExpenseType)
	
	component.createExpenseTypeWithNominalCode(expenseType, nominalCode.name)	
...
Rep.nextTestStep("Click onto [Delete]")
	...
	gAct.compareStringAndReport(delText, nominalCode.code + " | " + nominalCode.name)
	...
	if(modalError){
		...
		gAct.findSubstringInStringAndReport(errtext, nominalCode.name + " cannot be deleted:")
		gAct.findSubstringInStringAndReport(errtext, "At least one Expense Type refers to it")
	}
