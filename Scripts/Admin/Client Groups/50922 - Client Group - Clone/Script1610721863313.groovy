...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String clonedClientGroupName = Com.generateRandomText(10)
	String clonedClientGroupCode = clonedClientGroupName.toUpperCase()
	
	String[] newClonedClientGroup = [clonedClientGroupName, clonedClientGroupCode]
	ClientGroup clientGroupClone = GenerateClientGroup.createClientGroup(newClonedClientGroup)
...
Rep.nextTestStep("Click onto [Inline menu] against a client Group")
...
Rep.nextTestStep("Verify the [Name] is pre-populated")
	...
	gAct.compareStringAndReport(actName, clientGroup.name)
	gObj.setEditSync(Const.kip4Generic + "input_Name", clientGroupClone.name)
	
	
Rep.nextTestStep("Verify User is able to add [Code]")
	gObj.setEditSync(Const.kip4Generic + "input_Code", clientGroupClone.code)
	
	
Rep.nextTestStep("Verify the [Active Checkbox] is pre-selected as per the cloned Client Group")
	def activeRem = WebD.getCheckBoxState("//input[@id='active']")
	gAct.compareStringAndReport(activeRem, "true")

		
Rep.nextTestStep("Save Cloned Client Group")
	gObj.buttonClickSync(Const.kip4Generic + "input_Close_btn btn-primary saveclosebutton")
	Act.verifySavePopUpText()
	
	
Rep.nextTestStep("Verify the changes are updated accurately on the Client Group table")
	sVal.searchTableAddedValue(2, clientGroupClone.code)	
