...
Rep.nextTestStep("Verify the [Name], [Active] & [Symbol] fields are copied from the parent currency")
	...
	String currSym = gObj.getAttributeSync(Const.kip4Currency + "input_symbol", "value")
	gAct.compareStringAndReport(currSym, currency.symbol)
	
	
Rep.nextTestStep("Add a [Name]")
	gObj.setEditSync(Const.kip4Generic + "input_Name", clonedCurrency.name)
	
	
Rep.nextTestStep("Add a [Code] ")
	gObj.setEditSync(Const.kip4Generic + "input_Code", clonedCurrency.code)
...
Rep.nextTestStep("Click [Save & Close] button")
	...
	sVal.searchTableAddedValue(2, clonedCurrency.code)
