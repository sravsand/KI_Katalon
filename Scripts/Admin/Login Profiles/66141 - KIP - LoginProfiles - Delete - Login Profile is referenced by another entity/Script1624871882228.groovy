...
Rep.nextTestStep("KIP - V6 Stage Login page")
	...
	String[] newLogin = [name, code, userNme, resource.code, "TEst1234", loginProfile.code, ""]
	LoginPG login = GenerateLoginPG.createLoginPG(newLogin)
	component.createLoginWithLoginProfile(login, "Administration")
...
Rep.nextTestStep("Verify error")
	...
	if(modalError){
		...
		gAct.findSubstringInStringAndReport(errtext, "At least one User refers to it")
	}
...
Rep.nextTestStep("Verify the [deliverableType] still exists in the table")
	sVal.searchTableAddedValue(3, loginProfile.name)
